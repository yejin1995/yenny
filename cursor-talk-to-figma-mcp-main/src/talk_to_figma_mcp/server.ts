...
// Convert Annotations to Frames Tool
server.tool(
  "convert_annotations_to_frames",
  "Convert annotations on a node into editable Figma frames with text content",
  {
    nodeId: z.string().describe("The ID of the node whose annotations should be converted to frames"),
    offsetX: z.number().optional().default(400).describe("X offset for placing annotation frames"),
    offsetY: z.number().optional().default(0).describe("Y offset for placing annotation frames"),
    frameWidth: z.number().optional().default(300).describe("Width of each annotation frame"),
    frameSpacing: z.number().optional().default(20).describe("Spacing between annotation frames"),
  },
  async ({ nodeId, offsetX, offsetY, frameWidth, frameSpacing }: any) => {
    try {
      // Get annotations for the node
      const annotationsResult = await sendCommandToFigma("get_annotations", {
        nodeId,
        includeCategories: true,
      });
      
      const annotationsData = annotationsResult as {
        nodeId: string;
        name: string;
        annotations: Array<{ nodeId: string; annotation: any }>;
      ...
      if (!annotationsData.annotations || annotationsData.annotations.length === 0) {
        return {
          content: [
            {
              type: "text",
              text: "No annotations found on this node",
            },
          ],
        };
      }

      // Get the parent node to append frames to
      const nodeInfo = await sendCommandToFigma("get_node_info", { nodeId });
      const node = nodeInfo as { id: string; name: string; type: string; parentId?: string };
      const parentId = node.parentId || nodeId;

      const categoryMap: Record<string, { label: string; color: string }> = {};
      if (annotationsData.categories) {
        annotationsData.categories.forEach((cat) => {
          categoryMap[cat.id] = { label: cat.label, color: cat.color };
        });
      }

      // Create frames for each annotation
      const createdFrames: Array<{ id: string; name: string }> = [];
      let currentY = offsetY;

      for (let i = 0; i < annotationsData.annotations.length; i++) {
        const { annotation } = annotationsData.annotations[i];
        const category = annotation.categoryId ? categoryMap[annotation.categoryId] : null;
        const categoryLabel = category ? category.label : "Annotation";
        
        // Determine frame color based on category
        let frameColor = { r: 0.95, g: 0.95, b: 0.95, a: 1 }; // Default light gray
        if (category) {
          // Map category colors to RGB
          const colorMap: Record<string, { r: number; g: number; b: number }> = {
            green: { r: 0.9, g: 0.95, b: 0.9 },
            blue: { r: 0.9, g: 0.9, b: 0.95 },
            pink: { r: 0.95, g: 0.9, b: 0.95 },
            orange: { r: 0.95, g: 0.95, b: 0.9 },
          };
          const mappedColor = colorMap[category.color] || frameColor;
          frameColor = { ...mappedColor, a: 1 };
        }

        // Create frame for annotation
        const frameName = `주석 - ${categoryLabel} ${i + 1}`;
        const frameResult = await sendCommandToFigma("create_frame", {
          x: offsetX,
          y: currentY,
          width: frameWidth,
          height: 200, // Will auto-resize based on content
          name: frameName,
          parentId,
        ...
        const frame = frameResult as { id: string; name: string };
        ...
        // Create content text (remove markdown headers for cleaner display)
        const contentText = annotation.labelMarkdown
          .replace(/^##\s+/gm, "")
          .replace(/^#\s+/gm, "")
          .trim();
      ...
      return {
        content: [
          {
            type: "text",
            text: `Successfully converted ${createdFrames.length} annotations to editable frames:\n${createdFrames.map((f) => `- ${f.name} (ID: ${f.id})`).join("\n")}`,
          },
        ],
      };
    } catch (error) {
      return {
        content: [
          {
            type: "text",
            text: `Error converting annotations to frames: ${error instanceof Error ? error.message : String(error)}`,
          },
        ],
      };
    }
